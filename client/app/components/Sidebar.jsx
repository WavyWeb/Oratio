...
const NavItem = ({ href, icon, label, collapsed, isActiveOverride }) => {
  ...
  return (
    <Link href={href} className="block mb-1 group">
      <div className={`relative overflow-hidden rounded-xl transition-all duration-300 ease-[cubic-bezier(0.25,0.1,0.25,1)] ${isActive ? 'bg-amber-50 shadow-sm' : 'hover:bg-slate-50'}`}>
        ...
        <div className={`relative flex items-center p-2.5 text-sm font-medium transition-all duration-300 ${collapsed ? "justify-center" : "space-x-3"}`}>
          <div className={`transition-all duration-300 ${isActive ? 'text-rose-600' : 'text-slate-400 group-hover:text-slate-600'}`}>
          ...
          {!collapsed && (
            <span className={`whitespace-nowrap truncate max-w-[170px] transition-all duration-300 ${isActive ? 'text-slate-900 font-semibold' : 'text-slate-600 group-hover:text-slate-900'}`}>
              {label}
            </span>
          )}
        </div>
      ...
      >
        ...
        <div className="flex justify-end p-3 border-b border-gray-100">
          <button
            ...
            className="group p-1.5 rounded-lg hover:bg-slate-100 text-slate-400 hover:text-slate-800 transition-colors"
          >
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar flex flex-col pt-4 px-3">
          ...
          {isOpen && (
            <div className="mb-3 px-2 flex items-center justify-between">
              ...
              {isDemo && <span className="text-[10px] bg-amber-100 text-amber-600 px-1.5 py-0.5 rounded border border-amber-200">Demo</span>}
            </div>
          )}
          ...
          <div className="space-y-1">
            {sessions.map((session, idx) => (
              <NavItem
                key={session._id}
                href={`/report?report=${encodeURIComponent(JSON.stringify(session))}`}
                icon={<MessageSquare size={18} />}
                label={session.title || `Session ${idx + 1}`}
                collapsed={!isOpen}
                isActiveOverride={pathname.includes('report') && window.location.search.includes(encodeURIComponent(JSON.stringify(session).slice(0, 20)))} // Rough check for active
              />
            ))}
          </div>
          ...
          <Link href="/session" className="block group">
            {isOpen ? (
              <button className="w-full relative overflow-hidden rounded-xl bg-gradient-to-r from-amber-500 to-rose-500 p-[1px] shadow-md shadow-rose-500/20 transition-all hover:shadow-lg hover:shadow-rose-500/30 hover:scale-[1.02] active:scale-[0.98]">
                <div className="relative bg-white rounded-[11px] hover:bg-opacity-95 transition-colors">
                  <div className="flex items-center justify-center space-x-2 py-2.5 px-4">
                    <Plus size={18} className="text-rose-500" />
                    <span className="font-semibold text-sm text-slate-800">New Session</span>
                  </div>
                </div>
              </button>
            ) : (
              <button className="w-full h-12 flex justify-center items-center rounded-xl bg-white border border-rose-200 text-rose-500 hover:bg-rose-50 hover:border-rose-300 transition-all shadow-sm">
                <Plus size={24} />
              </button>
            )}
          </Link>
