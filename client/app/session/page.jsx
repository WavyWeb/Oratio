...
const WebRTCRecorder = () => {
  ...
  const handleSend = async () => {
    ...
    toast.info(
      <div>
        <p>Your recording is being analyzed...</p>
        <button onClick={() => router.push("/dashboard")} className="mt-2 btn-gradient text-white px-4 py-2 rounded-lg text-sm">
          Go to Dashboard
        </button>
      </div>,
    ...
    if (uploadedFile) {
      ...
      formData.append("file", blob, isVideo ? "recording.webm" : "recording.webm");
    }
    ...
    try {
      ...
      toast.success(
        <div>
          ...
          <button onClick={() => router.push("/report")} className="mt-2 btn-gradient text-white px-4 py-2 rounded-lg text-sm">
            View Report
          </button>
        </div>,
        { autoClose: false, closeOnClick: false }
  ...
  return (
    <DashboardLayout>
      ...
      ═══════════════════════════════════════════════ */}
      {phase === PHASE_SELECT && (
        <div className="flex min-h-[calc(100vh-6rem)] static-bg">
          <div className="flex-1 flex flex-col items-center justify-center px-4 py-10">
            {/* Logo */}
            <div className="mb-3">
              <Image src="/logo1.png" alt="Oratio" width={64} height={64} className="drop-shadow-lg" />
            </div>
            <h1 className="text-3xl md:text-4xl font-bold gradient-text mb-2">New Session</h1>
            <p className="text-white/50 text-sm mb-10">Choose how you'd like to record</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 w-full max-w-3xl">
              ...
              <button
                ...
                className="group relative flex flex-col items-center gap-4 p-8 rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm hover:border-amber-500/50 hover:bg-amber-500/10 transition-all duration-300 hover:scale-[1.03] active:scale-[0.98]"
              >
                <div className="w-16 h-16 rounded-full bg-gradient-to-br from-amber-500 to-orange-600 flex items-center justify-center shadow-lg shadow-amber-500/30 group-hover:shadow-amber-500/50 transition-shadow">
                  <Video size={28} className="text-white" />
                </div>
                <span className="text-white font-semibold text-lg">Video</span>
                <span className="text-white/40 text-xs">Record with camera</span>
              </button>
              ...
              >
                ...
                <span className="text-white/40 text-xs">Record with microphone</span>
              </button>
              ...
              >
                ...
                <span className="text-white/40 text-xs">Video or audio file</span>
                <input
                  ref={fileInputRef}
                  type="file"
                  accept="video/*,audio/*"
          ...
          <div className="flex items-center justify-center gap-6 px-6 py-6 bg-black/40">
            {!isRecording ? (
              <button
                ...
                className="flex items-center gap-3 px-10 py-4 rounded-full bg-gradient-to-br from-amber-500 to-orange-600 text-white font-semibold text-lg shadow-lg shadow-amber-500/40 transition-all duration-200 hover:scale-105 active:scale-95"
              >
              ...
              <>
                ...
                <button
                  ...
                  className="w-16 h-16 rounded-full bg-gradient-to-br from-amber-500 to-orange-600 flex items-center justify-center shadow-lg shadow-amber-500/40 transition-all duration-200 hover:scale-110 active:scale-95"
                >
        ...
        <div className="flex min-h-[calc(100vh-6rem)] static-bg">
          <div className="flex-1 flex flex-col items-center justify-center px-4 py-8">
            ...
            <div className="w-full max-w-3xl mb-6">
              <button onClick={backToSelect} className="text-white/50 hover:text-white transition-colors flex items-center gap-2 text-sm">
                <ArrowLeft size={18} /> Back
              </button>
            </div>

            <h2 className="text-2xl font-bold text-white mb-6">Preview your {mode === "video" ? "video" : "audio"}</h2>

            {/* Player */}
            <div className="w-full max-w-3xl rounded-2xl overflow-hidden border border-white/10 bg-black/30 shadow-2xl">
              {mode === "video" ? (
            ...
            <div className="flex items-center gap-4 mt-8">
              ...
              {!uploadedFile && (
                <button
                  onClick={reRecord}
                  className="px-6 py-3 rounded-xl border border-white/20 bg-white/5 hover:bg-white/10 text-white text-sm font-medium transition-all hover:scale-105 active:scale-95"
                >
                  Re-record
                </button>
              )}
              ...
              <button
                onClick={handleSend}
                disabled={loading}
                className="px-8 py-3 rounded-xl btn-gradient text-white text-sm font-semibold flex items-center gap-2 transition-all hover:scale-105 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
              >
                {loading ? (
                  <>
