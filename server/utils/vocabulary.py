        ...
        try:
            ...
            if "429" in error_str or "quota" in error_str.lower() or "rate" in error_str.lower():
                if attempt < max_retries - 1:
                    ...
                    print(f"⏳ Rate limit hit on vocabulary evaluation. Waiting {wait_time}s before retry {attempt + 2}/{max_retries}...")
                    time.sleep(wait_time)
                    continue
            print(f"Error generating vocabulary evaluation: {str(e)}")
            return "Unable to evaluate vocabulary at this time."
    
    return "Unable to evaluate vocabulary after multiple retries."
